...
export async function initHRDashboard() {
                ...
                <div class="card bg-vscode border-secondary shadow-sm h-100">
                    ...
                    <div class="card-body">
                        <div class="timeline-compact">
                            <div class="pb-3 border-start border-secondary position-relative ps-4 ms-2">
                                <div class="position-absolute start-0 translate-middle-x bg-primary rounded-circle" style="width: 10px; height: 10px; top: 5px;"></div>
                                <div class="small fw-bold text-white">Karyawan Baru Bergabung</div>
                                <div class="text-white-50 x-small">Ahmad Junaidi - Teknisi</div>
                                <div class="text-white-50 x-small mt-1 opacity-50">2 jam yang lalu</div>
                            </div>
                            <div class="pb-3 border-start border-secondary position-relative ps-4 ms-2">
                                <div class="position-absolute start-0 translate-middle-x bg-warning rounded-circle" style="width: 10px; height: 10px; top: 5px;"></div>
                                <div class="small fw-bold text-white">Pengajuan Izin Disetujui</div>
                                <div class="text-white-50 x-small">Siti Aminah - Admin</div>
                                <div class="text-white-50 x-small mt-1 opacity-50">5 jam yang lalu</div>
                            </div>
                            <div class="pb-3 border-start border-secondary position-relative ps-4 ms-2">
                                <div class="position-absolute start-0 translate-middle-x bg-success rounded-circle" style="width: 10px; height: 10px; top: 5px;"></div>
                                <div class="small fw-bold text-white">Update Gaji Periode April</div>
                                <div class="text-white-50 x-small">Proses perhitungan selesai</div>
                                <div class="text-white-50 x-small mt-1 opacity-50">Kemarin, 14:00</div>
                            </div>
                            <div class="position-relative ps-4 ms-2">
                                <div class="position-absolute start-0 translate-middle-x bg-info rounded-circle" style="width: 10px; height: 10px; top: 5px;"></div>
                                <div class="small fw-bold text-white">Training Rutin Selesai</div>
                                <div class="text-white-50 x-small">Modul Troubleshooting FO</div>
                                <div class="text-white-50 x-small mt-1 opacity-50">12 Apr 2026</div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    try {
        ...
        // Add Top Performers Section (Replaces or appends to Timeline or a new section)
        if (topRes.data && topRes.data.length > 0) {
            const timelineContainer = container.querySelector('.timeline-compact');
            if (timelineContainer) {
                // Prepend Top Performers to activity log or create separate panel
                // For now, let's inject a "Top Performance" header and items at the top of the right column
                const rightCol = container.querySelector('.col-lg-4 .card-body');
                if (rightCol) {
                    const topHtml = `
                        <div class="mb-4">
                    ...
                    `;
                    // Keep the existing timeline items but add the top performers above them
                    const existingTimelineItems = timelineContainer.innerHTML;
                    rightCol.innerHTML = topHtml + '<div class="timeline-compact">' + existingTimelineItems + '</div>';
                }
            }
        }
