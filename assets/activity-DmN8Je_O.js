import{A as Y,s as l}from"./config-CON8XM2G.js";/* empty css               */function Z(i,f,I="Map"){return!i||!f?"":`<a href="${`https://www.google.com/maps/search/?api=1&query=${i},${f}`}" target="_blank" class="badge bg-success bg-opacity-10 text-success text-decoration-none ms-1"><i class="bi bi-geo-alt-fill"></i> ${I}</a>`}document.addEventListener("DOMContentLoaded",async()=>{const i=document.getElementById("activity-chat-container"),f=document.getElementById("error-alert"),I=document.getElementById("error-message"),q=document.getElementById("technician-info"),A=document.getElementById("tech-name"),F=document.getElementById("tech-id");let o;document.getElementById("executionModal")&&(o=new bootstrap.Modal(document.getElementById("executionModal")));const d=document.getElementById("btn-save-exec"),C=document.getElementById("exec-status"),P=document.getElementById("exec-notes"),w=document.getElementById("exec-photo"),v=document.getElementById("exec-photo-preview"),y=document.getElementById("photo-preview-container"),B=document.getElementById("exec-mac"),$=document.getElementById("exec-sn"),T=document.getElementById("exec-actual-date"),M=document.getElementById("exec-activation-date"),S=document.getElementById("exec-cable-label"),H=document.getElementById("claim-section"),O=document.getElementById("execution-details-section"),j=document.getElementById("team-selection-container"),b=document.getElementById("btn-confirm-claim");let s=null,c=null;const R=document.getElementById("btn-refresh"),g=document.getElementById("btn-install");let r,p;window.addEventListener("beforeinstallprompt",e=>{e.preventDefault(),p=e,g&&g.classList.remove("d-none")}),R&&R.addEventListener("click",async()=>{r&&await h(r)}),g&&g.addEventListener("click",async()=>{if(p){p.prompt();const{outcome:e}=await p.userChoice;e==="accepted"&&g.classList.add("d-none"),p=null}}),w&&w.addEventListener("change",function(e){const t=e.target.files[0];if(t){const a=new FileReader;a.onload=function(n){c=n.target.result,v.src=c,y.classList.remove("d-none")},a.readAsDataURL(t)}else c=null,y.classList.add("d-none"),v.src=""});const D=new URLSearchParams(window.location.search).get("code");if(!D){k("Akses ditolak. Format URL tidak valid atau kode teknisi tidak ditemukan. Mengalihkan ke halaman login..."),setTimeout(()=>{window.location.href=Y+"/admin/login"},3e3);return}try{const{data:e,error:t}=await l.from("employees").select("*").eq("employee_id",D).single();if(t||!e){k(`Data teknisi dengan ID ${D} tidak ditemukan.`);return}A.innerText=e.name,F.innerText=e.employee_id,r=e.id,q.classList.remove("d-none"),await h(e.id)}catch(e){console.error(e),k("Terjadi kesalahan saat memuat data dari server.")}async function h(e){i.innerHTML=`
        ...
        `;const{data:t,error:a}=await l.from("work_orders").select(`
            ...
            `).or(`status.eq.confirmed,employee_id.eq.${e},claimed_by.eq.${e}`).order("created_at",{ascending:!0});if(a){k("Gagal memuat daftar pekerjaan: "+a.message),i.innerHTML="";return}if(!t||t.length===0){i.innerHTML=`
        ...
        `).join(""),b.onclick=async()=>{b.disabled=!0,b.innerHTML='<span class="spinner-border spinner-border-sm"></span> Memproses...';try{const a=Array.from(document.querySelectorAll(".team-member-check:checked")).map(m=>m.value),{error:n}=await l.from("work_orders").update({status:"open",claimed_by:r,claimed_at:new Date().toISOString(),ket:"Team Lead: "+A.innerText+(a.length>0?" | Anggota: "+a.length:"")}).eq("id",e.id);if(n)throw n;o.hide(),await h(r)}catch(a){alert("Gagal mengambil tiket: "+a.message)}finally{b.disabled=!1,b.innerText="Konfirmasi & Mulai Pengerjaan"}},o.show()}function K(e){if(!o)return;s=e,H.classList.add("d-none"),O.classList.remove("d-none"),document.getElementById("btn-save-exec").classList.remove("d-none"),P.value=e.ket||"",C.value=e.status==="closed"?"closed":"open",w.value="",c=null,y.classList.add("d-none"),v.src="";const t=e.installation_monitorings&&e.installation_monitorings.length>0?e.installation_monitorings[0]:null;t?(B.value=t.mac_address||"",$.value=t.sn_modem||"",S.value=t.cable_label||"",T.value=t.actual_date||"",M.value=t.activation_date||"",t.photo_proof&&(c=t.photo_proof,v.src=c,y.classList.remove("d-none"))):(B.value="",$.value="",S.value="",T.value=new Date().toISOString().split("T")[0],M.value=new Date().toISOString().split("T")[0]),o.show()}d&&d.addEventListener("click",async()=>{if(!s)return;const e=C.value,t=P.value;d.disabled=!0,d.innerHTML='<span class="spinner-border spinner-border-sm"></span> Menyimpan...';try{let a=s.points||0;if(e==="closed"){const{data:L}=await l.from("master_queue_types").select("base_point").eq("id",s.type_id).maybeSingle();L?a=L.base_point:a=100}const{error:n}=await l.from("work_orders").update({status:e,points:a,ket:t||s.ket}).eq("id",s.id);if(n)throw n;const m={work_order_id:s.id,customer_id:s.customer_id,employee_id:s.employee_id,mac_address:B.value||null,sn_modem:$.value||null,cable_label:S.value||null,actual_date:T.value||null,activation_date:M.value||null,photo_proof:c,notes:t,is_confirmed:!1},x=s.installation_monitorings&&s.installation_monitorings.length>0?s.installation_monitorings[0]:null;x&&(m.id=x.id);const{error:_}=await l.from("installation_monitorings").upsert(m,{onConflict:"work_order_id"});if(_)throw _;o.hide(),await h(r)}catch(a){console.error("Update error:",a),alert("Gagal mengupdate status: "+a.message)}finally{d.disabled=!1,d.innerText="Simpan"}});function k(e){f.classList.remove("d-none"),I.innerText=e,i.innerHTML=""}function J(e){const t=e?e.toLowerCase():"";return t==="waiting"?"warning":t==="confirmed"?"success":t==="open"?"info":t==="closed"?"secondary":t.includes("pending")||t.includes("antrian")?"warning":t.includes("selesai")?"success":t.includes("proses")?"info":t.includes("cancel")?"danger":"secondary"}});
