import{s as K}from"./config-CON8XM2G.js";import{g as M,c as T,p as D,d as F,b as C}from"./ui-common-BJBdVW0F.js";import"./auth-service-BA7j2u7-.js";async function O(){const k=document.getElementById("add-customer-view-container"),p=document.getElementById("back-to-customers-btn");p&&(p.onclick=()=>{window.switchAdminModule&&window.switchAdminModule("customers-content")}),k.innerHTML=`
        <div class="row">
    ...
    `;const v=document.getElementById("adv-reg-id-display"),i=document.getElementById("adv-reg-password"),g=document.getElementById("adv-btn-gen-credentials"),u=document.getElementById("adv-toggle-pass"),x=document.getElementById("adv-cust-ktp-file"),I=document.getElementById("ktp-preview"),B=document.getElementById("ktp-preview-container"),E=document.getElementById("adv-cust-house-file"),P=document.getElementById("house-preview"),N=document.getElementById("house-preview-container"),n=document.getElementById("adv-cust-lat"),c=document.getElementById("adv-cust-lng");let l=null,f=null,h=null;g&&(g.onclick=()=>{const e=M(),a=T();i.value=e,v.innerText=a,v.classList.add("text-info"),l={pass:e,code:a}}),u&&(u.onclick=()=>{const e=u.querySelector("i");i.type==="password"?(i.type="text",e.classList.replace("bi-eye","bi-eye-slash")):(i.type="password",e.classList.replace("bi-eye-slash","bi-eye"))});const w=(e,a,s,t)=>{e.onchange=r=>{const m=r.target.files[0];if(m){const d=new FileReader;d.onload=y=>{t(y.target.result),a.src=y.target.result,s.classList.remove("d-none")},d.readAsDataURL(m)}}};w(x,I,B,e=>f=e),w(E,P,N,e=>h=e),D("adv-cust-package");let o,b;const A=[-7.15097,112.721245];setTimeout(()=>{o&&o.remove(),o=L.map("location-picker-map").setView(A,13),L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png").addTo(o);const e=(a,s)=>{b?b.setLatLng([a,s]):b=L.marker([a,s]).addTo(o),o.panTo([a,s]);const t=document.getElementById("adv-google-maps-link");if(t){const r=C(a,s);t.href=r,t.classList.remove("d-none")}};o.on("click",a=>{const{lat:s,lng:t}=a.latlng;n.value=s.toFixed(7),c.value=t.toFixed(7),e(s,t)}),[n,c].forEach(a=>{a.oninput=()=>{const s=parseFloat(n.value),t=parseFloat(c.value);!isNaN(s)&&!isNaN(t)&&e(s,t)}})},500),document.getElementById("save-adv-customer-btn").onclick=async()=>{if(!l)return alert('Silakan klik "Buat Kredensial Otomatis" terlebih dahulu.');const e={name:document.getElementById("adv-cust-name").value.trim(),ktp:document.getElementById("adv-cust-ktp").value.trim(),phone:document.getElementById("adv-cust-phone").value.trim(),alt_phone:document.getElementById("adv-cust-alt-phone").value.trim(),packet:document.getElementById("adv-cust-package").value,address:document.getElementById("adv-cust-address").value.trim(),lat:parseFloat(n.value)||null,lng:parseFloat(c.value)||null,photo_ktp:f,photo_rumah:h,customer_code:l.code};if(!e.name||!e.phone||!e.address)return alert("Nama, No. HP, dan Alamat wajib diisi.");const a=document.getElementById("save-adv-customer-btn"),s=a.innerHTML;a.disabled=!0,a.innerHTML='<span class="spinner-border spinner-border-sm me-2"></span>Mendaftarkan...';try{const t=document.getElementById("adv-cust-email").value.trim(),m=(await F(l.code,l.pass,l.code,t)).user.id,{error:d}=await K.from("customers").update(e).eq("id",m);if(d)throw d;alert(`Sukses! Pelanggan "${e.name}" berhasil didaftarkan.

ID LOGIN: ${l.code}
PASSWORD: ${l.pass}
KODE PELANGGAN: ${l.code}`),window.switchAdminModule&&window.switchAdminModule("customers-content")}catch(t){console.error("Save customer error:",t),alert("Terjadi kesalahan: "+t.message)}finally{a.disabled=!1,a.innerHTML=s}}}export{O as initAddCustomerView};
