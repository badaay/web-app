const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/index-BCNb7kqE.js","assets/config-CON8XM2G.js","assets/toast-D3E5iWRc.js","assets/ui-common-hIOATXsD.js","assets/utils-CJqnORQj.js"])))=>i.map(i=>d[i]);
import{_ as i}from"./admin-LaNO8hp3.js";import"./auth-service-CtC7mI0P.js";import"./config-CON8XM2G.js";import"./config-Dipwna8v.js";import"./toast-D3E5iWRc.js";async function _(){const e=document.getElementById("role-feature");e&&(e.innerHTML=`
    ...
    `,document.getElementById("quick-add-customer-btn").onclick=()=>{document.dispatchEvent(new CustomEvent("navigate",{detail:"add-customer-view-content"}))},document.getElementById("quick-wo-btn").onclick=()=>{document.dispatchEvent(new CustomEvent("navigate",{detail:"work-orders-content"}))},document.getElementById("quick-map-btn").onclick=()=>{document.dispatchEvent(new CustomEvent("navigate",{detail:"customer-map-view-content"}))},g(),w())}async function g(){try{const{supabase:e}=await i(async()=>{const{supabase:n}=await import("./config-CON8XM2G.js").then(c=>c.b);return{supabase:n}},[]),[m,a,o,d]=await Promise.all([e.from("customers").select("*",{count:"exact",head:!0}),e.from("work_orders").select("*",{count:"exact",head:!0}).neq("status","closed"),e.from("employees").select("*",{count:"exact",head:!0}).eq("status","Aktif"),e.from("internet_packages").select("*",{count:"exact",head:!0})]);document.getElementById("stat-total-customers").innerText=m.count||0,document.getElementById("stat-active-wo").innerText=a.count||0,document.getElementById("stat-total-employees").innerText=o.count||0,document.getElementById("stat-total-packages").innerText=d.count||0}catch(e){console.error("Dashboard stats error:",e)}}async function w(){const e=document.querySelector(".container-fluid.py-4");if(!e)return;e.insertAdjacentHTML("beforeend",`
        <div class="row g-4 mt-2">
            <div class="col-12">
    ...
    `),document.getElementById("view-all-wo-link").onclick=()=>{document.dispatchEvent(new CustomEvent("navigate",{detail:"work-orders-content"}))};const a=document.getElementById("today-queue-list"),{supabase:o}=await i(async()=>{const{supabase:t}=await import("./config-CON8XM2G.js").then(s=>s.b);return{supabase:t}},[]),{getStatusColor:d,getStatusDisplayText:n}=await i(async()=>{const{getStatusColor:t,getStatusDisplayText:s}=await import("./utils-CJqnORQj.js");return{getStatusColor:t,getStatusDisplayText:s}},[]),c=new Date().toISOString().split("T")[0],{data:v,error:u}=await o.from("work_orders").select("*, customers(name, phone), employees!employee_id(name), master_queue_types(name, color, icon)").or(`registration_date.eq.${c},status.in.(confirmed,open)`).order("created_at",{ascending:!1}).limit(20);if(u){a.innerHTML=`<div class="p-3 text-danger">Error: ${u.message}</div>`;return}if(v.length===0){a.innerHTML=`
            <div class="text-center p-5">
                <i class="bi bi-check2-circle fs-1 text-success"></i>
                <p class="mt-2 text-white-50">Tidak ada antrian aktif untuk hari ini. Santai!</p>
            </div>
        `;return}a.innerHTML=v.map(t=>{const s=t.master_queue_types,r=t.customers,l=t.employees,p=d(t.status),b=n(t.status);return`
        ...
        `}).join(""),a.querySelectorAll(".dashboard-confirm-wo").forEach(t=>{t.onclick=async()=>{const s=t.dataset.woId,{initWorkOrders:r}=await i(async()=>{const{initWorkOrders:l}=await import("./index-BCNb7kqE.js");return{initWorkOrders:l}},__vite__mapDeps([0,1,2,3,4]));document.dispatchEvent(new CustomEvent("request-wo-confirmation",{detail:{woId:s}}))}})}export{_ as initDashboard};
