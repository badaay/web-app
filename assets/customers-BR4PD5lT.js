import{s as T,A as N}from"./config-CON8XM2G.js";import{g as _,a as A}from"./registration-service-BCvO0A0I.js";import{g as H,s as q,a as E,p as C}from"./ui-common-hIOATXsD.js";import{s as v}from"./toast-D3E5iWRc.js";import"./auth-service-PfjuabZR.js";async function G(){const y=document.getElementById("customers-list"),B=document.getElementById("add-customer-view-btn");let k=[];B&&(B.onclick=()=>{document.dispatchEvent(new CustomEvent("navigate",{detail:"add-customer-view-content"}))});async function I(){y.innerHTML=H("Memuat pelanggan...");const{data:t,error:c}=await T.from("customers").select("*, roles(name)").order("created_at",{ascending:!1});if(c){y.innerHTML=`<div class="text-danger">Kesalahan: ${c.message}</div>`;return}if(k=t,t.length===0){y.innerHTML=`
        ...
        `,document.querySelectorAll(".view-map").forEach(e=>{e.onclick=s=>{s.preventDefault(),s.stopPropagation();const l=k.find(u=>String(u.id)===e.dataset.id);S(parseFloat(e.dataset.lat),parseFloat(e.dataset.lng),l)}}),document.querySelectorAll(".edit-cust").forEach(e=>{e.onclick=s=>{s.preventDefault(),s.stopPropagation(),F(k.find(l=>String(l.id)===e.dataset.id))}}),document.querySelectorAll(".quick-repair").forEach(e=>{e.onclick=()=>{const s=k.find(l=>String(l.id)===e.dataset.id);document.dispatchEvent(new CustomEvent("navigate",{detail:"work-orders-content"})),setTimeout(()=>{document.dispatchEvent(new CustomEvent("quick-wo",{detail:s}))},500)}}),document.querySelectorAll(".reset-pass").forEach(e=>{e.onclick=async s=>{s.preventDefault(),s.stopPropagation();const l=e.dataset.id,u=e.dataset.name;if(!confirm(`Apakah Anda yakin ingin mengatur ulang kata sandi untuk "${u}"?`))return;const g=_(),o=e.querySelector("i"),$=o.className;try{e.disabled=!0,o.className="spinner-border spinner-border-sm",await A(l,g),v("success",`Kata sandi berhasil diatur ulang!

User: ${u}
Password Baru: ${g}`)}catch(f){console.error("Reset password error:",f),v("error","Gagal mengatur ulang kata sandi: "+f.message)}finally{e.disabled=!1,o.className=$}}})}function S(t,c,e){if(!t||!c)return v("warning","Koordinat tidak disetel untuk pelanggan ini.");const s=E(t,c),l=`
        ...
        `;q(t,c,`Lokasi: ${e?.name}`,l)}async function F(t=null){const c=new bootstrap.Modal(document.getElementById("crudModal")),e=document.getElementById("crudModalTitle"),s=document.getElementById("crudModalBody"),l=document.getElementById("save-crud-btn");e.innerText=t?`Edit Pelanggan: ${t.name}`:"Tambah Pelanggan",s.innerHTML=`
                         ...
                         <div class="row g-3">
                            ...
                            <div class="col-12">
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    ...
                                    <a id="edit-google-maps-link" href="${t?.lat?E(t.lat,t.lng):"#"}" target="_blank" class="btn btn-link text-success p-0 text-decoration-none smaller ${t?.lat?"":"d-none"}">
        ...
        `,setTimeout(()=>{const a=document.getElementById("cust-lat"),m=document.getElementById("cust-lng"),d=document.getElementById("edit-google-maps-link"),x=parseFloat(a.value)||-6.2,h=parseFloat(m.value)||106.8166,p=L.map("edit-location-picker-map").setView([x,h],14);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png").addTo(p);let b;parseFloat(a.value)&&parseFloat(m.value)&&(b=L.marker([x,h]).addTo(p));const w=(r,i)=>{b?b.setLatLng([r,i]):b=L.marker([r,i]).addTo(p),p.panTo([r,i]);const n=E(r,i);n&&(d.href=n,d.classList.remove("d-none"))};p.on("click",r=>{const{lat:i,lng:n}=r.latlng;a.value=i.toFixed(7),m.value=n.toFixed(7),w(i,n)}),[a,m].forEach(r=>{r.oninput=()=>{const i=parseFloat(a.value),n=parseFloat(m.value);!isNaN(i)&&!isNaN(n)&&w(i,n)}}),setTimeout(()=>p.invalidateSize(),300)},500),C("cust-packet-select",t?.packet);const u=document.getElementById("btn-gen-new-pass"),g=document.getElementById("toggle-new-pass"),o=document.getElementById("cust-new-password"),$=document.getElementById("new-pass-msg");u&&(u.onclick=()=>{o.value=_(),o.type="text",$.classList.remove("d-none")}),g&&(g.onclick=()=>{const a=g.querySelector("i");o.type==="password"?(o.type="text",a.classList.replace("bi-eye","bi-eye-slash")):(o.type="password",a.classList.replace("bi-eye-slash","bi-eye"))});let f=t?.photo_ktp||null,P=t?.photo_rumah||null;const M=(a,m,d,x)=>{const h=document.getElementById(a);h&&(h.onchange=p=>{const b=p.target.files[0];if(b){const w=new FileReader;w.onload=r=>{const i=r.target.result;x(i);const n=document.getElementById(d);n.classList.remove("d-none"),n.querySelector("img").src=i},w.readAsDataURL(b)}})};M("cust-ktp-file","edit-ktp-preview","edit-ktp-preview-container",a=>f=a),M("cust-house-file","edit-house-preview","edit-house-preview-container",a=>P=a),l.onclick=async()=>{const a={name:document.getElementById("cust-name").value.trim(),customer_code:document.getElementById("cust-code").value.trim(),ktp:document.getElementById("cust-ktp").value.trim(),phone:document.getElementById("cust-phone").value.trim(),alt_phone:document.getElementById("cust-alt-phone").value.trim(),packet:document.getElementById("cust-packet-select").value,install_date:document.getElementById("cust-install-date").value||null,address:document.getElementById("cust-address").value.trim(),username:document.getElementById("cust-username").value.trim(),mac_address:document.getElementById("cust-mac").value.trim(),damping:document.getElementById("cust-damping").value.trim(),lat:parseFloat(document.getElementById("cust-lat").value)||null,lng:parseFloat(document.getElementById("cust-lng").value)||null,photo_ktp:f,photo_rumah:P,email:document.getElementById("cust-email").value.trim()||(t?.email&&!t?.email.includes("@sifatih.id")?t.email:null)};if(!a.name||!a.address)return v("warning","Nama dan Alamat wajib diisi.");l.disabled=!0;const m=l.innerHTML;l.innerHTML='<span class="spinner-border spinner-border-sm me-2"></span>Menyimpan...';try{o.value.trim().length>0&&(await A(t.id,o.value.trim()),console.log("Password updated successfully"));const{error:d}=await T.from("customers").update(a).eq("id",t.id);if(d)throw d;v("success","Data pelanggan berhasil diperbarui!"),c.hide(),I()}catch(d){console.error("Save edit error:",d),v("error","Gagal menyimpan: "+d.message)}finally{l.disabled=!1,l.innerHTML=m}},c.show()}I()}export{G as initCustomers};
