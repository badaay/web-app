import{s as m}from"./supabase-BeOTOPRS.js";async function x(){const s=document.getElementById("customers-list"),c=document.getElementById("add-customer-view-btn"),p=document.getElementById("view-all-customers-map-btn");let i=[];c&&(c.onclick=()=>{window.switchAdminModule&&window.switchAdminModule("add-customer-view-content")}),p&&(p.onclick=()=>v());async function u(){s.innerHTML='<div class="text-center py-4"><div class="spinner-border text-primary" role="status"></div><div class="mt-2 text-white-50">Memuat pelanggan...</div></div>';const{data:e,error:a}=await m.from("customers").select("*, roles(name)").order("created_at",{ascending:!1});if(a){s.innerHTML=`<div class="text-danger">Kesalahan: ${a.message}</div>`;return}if(i=e,e.length===0){s.innerHTML='<div class="text-muted text-center py-4">Tidak ada pelanggan ditemukan.</div>';return}s.innerHTML=`
