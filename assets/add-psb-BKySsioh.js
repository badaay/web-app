import{s as E,A as G}from"./config-CON8XM2G.js";/* empty css               *//* empty css              *//* empty css               */import{A as U}from"./auth-service-BA7j2u7-.js";document.addEventListener("DOMContentLoaded",async()=>{const r=document.getElementById("psb-form-container");if(!r)return;const o=(t,s)=>{const e=document.getElementById(t);if(!e)return;const i=e.parentElement.querySelector(".custom-inline-error");i&&i.remove();const a=document.createElement("div");if(a.className="custom-inline-error mt-1 fw-bold shadow",a.style.background="#da3633",a.style.color="#fff",a.style.padding="6px 12px",a.style.borderRadius="6px",a.style.fontSize="0.8rem",a.style.position="absolute",a.style.zIndex="1050",a.style.animation="errorFadeIn 0.3s ease",a.innerHTML='<i class="bi bi-exclamation-circle-fill me-1"></i> '+s,e.parentElement.style.position="relative",e.tagName==="INPUT"||e.tagName==="TEXTAREA"){e.classList.add("is-invalid"),e.parentElement.insertBefore(a,e.nextSibling);const l=()=>{e.classList.remove("is-invalid"),a.parentNode&&a.remove()};e.addEventListener("input",l,{once:!0}),e.addEventListener("change",l,{once:!0}),e.focus()}else{a.style.left="50%",a.style.bottom="10%",a.style.transform="translate(-50%, 0)",e.appendChild(a);const l=()=>{a.parentNode&&a.remove()};e.addEventListener("click",l,{once:!0}),setTimeout(l,4e3),e.scrollIntoView({behavior:"smooth",block:"center"})}},p=()=>`
    ...
    `;r.innerHTML=p()+'<form id="add-psb-registration-form">'+n()+d()+j()+"</form>";const T=(t,s,e,i)=>{const a=document.getElementById(t),l=document.getElementById(s),v=document.getElementById(e),x=v.querySelector("img"),B=document.getElementById(i);a&&(a.addEventListener("change",function(S){const g=S.target.files[0];if(g){const f=new FileReader;f.onload=function(k){x.src=k.target.result,l.style.display="none",v.style.display="block"},f.readAsDataURL(g)}}),B.addEventListener("click",function(){a.value="",l.style.display="block",v.style.display="none",x.src=""}))};T("adv-cust-foto-rumah","drop-zone-rumah","preview-rumah","btn-remove-rumah"),T("adv-cust-foto-ktp","drop-zone-ktp","preview-ktp","btn-remove-ktp");const y=document.getElementById("adv-cust-phone"),u=document.getElementById("phone-duplicate-msg");y&&y.addEventListener("blur",async()=>{const t=y.value.trim();if(t.length<10)return;u.innerHTML='<span class="spinner-border spinner-border-sm"></span> Mengecek...',u.classList.remove("d-none","text-danger","text-success");const{data:s,error:e}=await E.from("customers").select("id").eq("phone",t).maybeSingle();s?(u.innerHTML='<i class="bi bi-exclamation-triangle-fill"></i> Nomor ini sudah terdaftar sebagai pelanggan.',u.classList.add("text-danger"),y.classList.add("is-invalid")):(u.innerHTML='<i class="bi bi-check-circle-fill"></i> Nomor tersedia.',u.classList.add("text-success"),y.classList.remove("is-invalid"))});let m,I;const q=[-7.15097,112.721245],A=[document.getElementById("step-1"),document.getElementById("step-2"),document.getElementById("step-3")],z=[document.getElementById("ind-step-1"),document.getElementById("ind-step-2"),document.getElementById("ind-step-3")],N=document.getElementById("step-loading");function C(t=500){return new Promise(s=>{A.forEach(e=>e.classList.add("d-none")),N.classList.remove("d-none"),setTimeout(()=>{N.classList.add("d-none"),s()},t)})}function K(t){z.forEach((s,e)=>{const i=s.querySelector(".step-num");e===t?(s.classList.remove("opacity-50"),i.classList.replace("bg-secondary","bg-primary"),i.classList.contains("bg-success")&&i.classList.replace("bg-success","bg-primary"),i.innerHTML=e+1):e<t?(s.classList.remove("opacity-50"),i.classList.contains("bg-secondary")&&i.classList.replace("bg-secondary","bg-success"),i.classList.contains("bg-primary")&&i.classList.replace("bg-primary","bg-success"),i.innerHTML='<i class="bi bi-check"></i>'):(s.classList.add("opacity-50"),i.classList.contains("bg-primary")&&i.classList.replace("bg-primary","bg-secondary"),i.classList.contains("bg-success")&&i.classList.replace("bg-success","bg-secondary"),i.innerHTML=e+1)})}async function _(t){if(await C(),A[t].classList.remove("d-none"),K(t),t===1){const s=L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"),e=L.tileLayer("https://tiles.stadiamaps.com/tiles/alidade_satellite/{z}/{x}/{y}{r}.{ext}",{minZoom:0,maxZoom:20,attribution:'&copy; CNES, Distribution Airbus DS, © Airbus DS, © PlanetObserver (Contains Copernicus Data) | &copy; <a href="https://www.stadiamaps.com/" target="_blank">Stadia Maps</a> &copy; <a href="https://openmaptiles.org/" target="_blank">OpenMapTiles</a> &copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors',ext:"jpg"}),i={Default:s,Terrains:e};m||(m=L.map("location-picker-map").addLayer(s).setView(q,13),L.control.layers(i).addTo(m),m.on("click",a=>D(a.latlng.lat,a.latlng.lng))),setTimeout(()=>m.invalidateSize(),200)}else if(t===2){const s=document.getElementById("summary-package-card");s.style.display="block",s.animate([{opacity:0,transform:"translateY(-10px)"},{opacity:1,transform:"translateY(0)"}],{duration:300,fill:"forwards"})}}function h(t,s,e){const i=document.getElementById(t);i&&(i.onclick=async()=>{e&&!e()||await _(s)})}h("btn-next-2",1,()=>document.getElementById("selected-package-name").value?!0:(o("package-cards-container","Silakan pilih paket internet terlebih dahulu."),!1)),h("btn-next-3",2,()=>{const t=document.getElementById("adv-cust-lat").value,s=document.getElementById("adv-cust-lng").value,e=document.getElementById("adv-cust-address").value.trim();let i=!0;return(!t||!s)&&(o("location-picker-map","Silakan pilih titik lokasi pemasangan di peta."),i=!1),e||(o("adv-cust-address","Silakan isi detail alamat pemasangan."),i=!1),i}),h("btn-back-1",0),h("btn-back-2",1);const R=document.getElementById("adv-cust-install-date");R&&(R.value=new Date().toISOString().split("T")[0]),await Y();function D(t,s){document.getElementById("adv-cust-lat").value=t.toFixed(7),document.getElementById("adv-cust-lng").value=s.toFixed(7),I&&I.remove(),I=L.marker([t,s]).addTo(m),m.setView([t,s],16)}const c=document.getElementById("btn-get-location");c&&(c.onclick=()=>{if(!navigator.geolocation)return alert("Browser Anda tidak mendukung geolokasi.");c.innerHTML='<span class="spinner-border spinner-border-sm me-1"></span> Mencari...',c.disabled=!0,navigator.geolocation.getCurrentPosition(t=>{D(t.coords.latitude,t.coords.longitude),c.innerHTML='<i class="bi bi-geo-alt-fill text-accent me-1"></i> Lokasi Saya',c.disabled=!1},t=>{alert("Gagal mendapatkan lokasi: "+t.message),c.innerHTML='<i class="bi bi-geo-alt-fill text-accent me-1"></i> Lokasi Saya',c.disabled=!1},{enableHighAccuracy:!0,timeout:1e4})});const b=document.getElementById("save-adv-customer-btn");b&&(b.onclick=async()=>{const t=document.getElementById("adv-cust-name").value.trim(),s=document.getElementById("adv-cust-phone").value.trim(),e=document.getElementById("adv-cust-foto-rumah").files[0],i=document.getElementById("adv-cust-foto-ktp").files[0];let a=!0;if(t||(o("adv-cust-name","Nama Lengkap wajib diisi."),a=!1),s||(o("adv-cust-phone","No. Handphone wajib diisi."),a=!1),e||(o("drop-zone-rumah","Foto Rumah wajib diunggah."),a=!1),i||(o("drop-zone-ktp","Foto KTP wajib diunggah."),a=!1),!a)return;const l=document.getElementById("selected-package-name").value,v=document.getElementById("adv-cust-address").value.trim(),x=document.getElementById("adv-cust-lat").value,B=document.getElementById("adv-cust-lng").value,S=null,g=document.getElementById("adv-cust-email").value.trim(),f=document.getElementById("adv-cust-alt-phone").value.trim();let k=v;(g||f)&&(k+=`
(Email: `+(g||"-")+" | HP Alt: "+(f||"-")+")"),b.innerHTML='<span class="spinner-border spinner-border-sm me-2"></span>Menyimpan Data...',b.disabled=!0;try{const{data:w}=await E.from("roles").select("id").eq("name","Customer").single(),O=w?w.id:null,F={name:t,address:k,phone:s,packet:l,install_date:S||null,lat:parseFloat(x),lng:parseFloat(B),role_id:O},V=g||`${s}@fatih.com`,{data:$,error:M}=await U.registerCustomer(V,"fatih1234",F);if(M)throw M;const P=$.customer;if(P&&P.id){const{error:H}=await E.from("work_orders").insert([{customer_id:P.id,status:"waiting",source:"customer",title:"Pemasangan Baru (PSB)",registration_date:new Date().toISOString().split("T")[0],referral_name:"",ket:"Paket: "+l,created_at:new Date().toISOString()}]);H&&console.warn("Notice: Gagal membuat antrian PSB otomatis:",H.message)}alert("Registrasi Pelanggan berhasil diselesaikan! Data masuk ke Antrian."),window.location.href=G+"/?success=true"}catch(w){alert("Gagal memproses pendaftaran: "+w.message),b.innerHTML='<i class="bi bi-check-circle-fill me-2"></i> Selesaikan Registrasi',b.disabled=!1}})});async function Y(){const r=document.getElementById("package-cards-container"),{data:o,error:p}=await E.from("internet_packages").select("*").order("price",{ascending:!0});if(p||!o||o.length===0){r.innerHTML='<div class="col-12 text-center text-white-50 my-4">Belum ada paket tersedia di database.</div>';return}r.innerHTML=o.map(n=>`
