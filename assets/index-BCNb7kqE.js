import{s as c}from"./config-CON8XM2G.js";import{s as m}from"./toast-D3E5iWRc.js";import{g as h}from"./ui-common-hIOATXsD.js";import{getStatusColor as b,getStatusDisplayText as v}from"./utils-CJqnORQj.js";async function x(e,a){if(!e)return;e.innerHTML=h("Memuat Antrian...");const{data:n,error:t}=await c.from("work_orders").select("*, customers(*), employees!employee_id(name)").order("created_at",{ascending:!1});if(t){e.innerHTML=`<div class="alert alert-danger">Error: ${t.message}</div>`;return}a(n)}function I(e,a,n){const t=document.getElementById("wo-status-summary");if(!t)return;const o=e.reduce((i,d)=>(i[d.status]=(i[d.status]||0)+1,i),{}),s=["waiting","confirmed","open","closed"],r=e.length,l=`
        <div class="d-flex justify-content-between align-items-center mb-3" style="flex-wrap: wrap;">
            ...
            <div class="ms-auto mt-2 mt-md-0">
                <button class="badge p-2 border-0 wo-filter-badge ${a==="All"?"ring-active":""}"
                    ...
                    data-filter="All"
                    title="Show all">
                    <i class="bi bi-list me-1"></i>Semua<br><small>${r}</small>
                </button>
            </div>
        </div>
    ...
    `,a.parentNode.insertBefore(n,a);const t=document.getElementById("wo-search-input"),o=document.getElementById("wo-clear-search");t.addEventListener("input",s=>e(s.target.value.toLowerCase())),o.addEventListener("click",()=>{t.value="",e("")})}function _(e,a,n){let t=e;return a!=="All"&&(t=t.filter(o=>o.status===a)),n&&(t=t.filter(o=>{const s=o.customers?.name?.toLowerCase()||"",r=o.customers?.phone?.toLowerCase()||"",l=o.employees?.name?.toLowerCase()||"",i=o.ket?.toLowerCase()||"";return s.includes(n)||r.includes(n)||l.includes(n)||i.includes(n)})),t}function S(e,a,n){const t=document.getElementById("work-orders-list");if(!t)return;if(e.length===0){t.innerHTML='<div class="alert alert-info mt-3">Tidak ada antrian yang cocok.</div>';return}const o=`
    ...
    `,t.querySelectorAll("[data-type-id]").forEach(o=>{o.addEventListener("click",async()=>{const s=o.dataset.typeId,r=o.dataset.typeName;await P(s,r,e)})}),document.getElementById("save-crud-btn").style.display="none"}async function P(e,a,n){const t=document.getElementById("crudModalBody");t.innerHTML=h("Memuat Form...");const[o,s,r]=await Promise.all([c.from("customers").select("id, name, phone, address"),c.from("employees").select("id, name"),c.from("internet_packages").select("id, name, price")]),l=o.data||[];s.data;const i=r.data||[],d=H(e);let p=`
        <form id="type-form">
    ...
    `;p+=`
            <div class="mb-3">
                ...
                <select class="form-select form-select-sm" name="customer_id" required>
                    ...
                    ${l.map(u=>`<option value="${u.id}">${u.name} (${u.phone})</option>`).join("")}
                </select>
            </div>
        `,d.package&&(p+=`
            <div class="mb-3">
        ...
        <form id="edit-form">
            ...
            <div class="mb-3">
                ...
                <select class="form-select form-select-sm" name="package_id">
                    <option value="">-- Pilih --</option>
                    ${r.map(i=>`<option value="${i.id}" ${e.package_id===i.id?"selected":""}>${i.name}</option>`).join("")}
                </select>
            </div>
    ...
    `}async function q(e,a){const n=document.getElementById("edit-form");if(!n.checkValidity()){n.reportValidity();return}const t=new FormData(n),o={title:t.get("title"),status:t.get("status"),customer_id:t.get("customer_id")||null,employee_id:t.get("employee_id")||null,package_id:t.get("package_id")||null,ket:t.get("ket"),payment_status:t.get("payment_status"),updated_at:new Date().toISOString()},{error:s}=await c.from("work_orders").update(o).eq("id",e);if(s){m(`Error: ${s.message}`,"error");return}m("Antrian berhasil diperbarui","success"),a.hide(),window.location.reload()}async function W(e,a){const n=new bootstrap.Modal(document.getElementById("crudModal"));document.getElementById("crudModalTitle").textContent=`Pantau Instalasi - ${e.customers?.name||"Customer"}`;const t=document.getElementById("crudModalBody");t.innerHTML=h("Memuat Data Monitoring..."),n.show();const{data:o,error:s}=await c.from("installation_monitorings").select("*").eq("work_order_id",e.id).single();if(s&&s.code!=="PGRST116"){m("Error loading monitoring data","error"),n.hide();return}const r=o||{};t.innerHTML=`
        <form id="monitoring-form">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control form-control-sm" name="notes" rows="3" placeholder="Catatan dari lapangan...">${r.notes||""}</textarea>
            </div>
    ...
    `;const l=document.getElementById("monitor-photo-input"),i=document.getElementById("photo-preview");l.addEventListener("change",d=>{i.innerHTML="",Array.from(d.target.files).forEach(p=>{if(p.size>5*1024*1024){m(`${p.name} terlalu besar (max 5MB)`,"warning");return}const u=new FileReader;u.onload=w=>{const k=document.createElement("div");k.className="col-auto",k.innerHTML=`
                    <div style="width:80px;height:80px;border-radius:4px;overflow:hidden;border:1px solid #444;">
                        <img src="${w.target.result}" style="width:100%;height:100%;object-fit:cover;">
                    </div>
                `,i.appendChild(k)},u.readAsDataURL(p)})}),document.getElementById("save-crud-btn").style.display="block",document.getElementById("save-crud-btn").onclick=async()=>{await j(e.id,r.id,l.files,a,n)}}async function j(e,a,n,t,o){const s=document.getElementById("monitoring-form"),r=new FormData(s),l=[];if(n&&n.length>0)for(let d=0;d<n.length;d++){const p=n[d],u=await N(p);l.push(u)}const i={work_order_id:e,status:r.get("status"),notes:r.get("notes"),photos:l.length>0?l:null,updated_at:new Date().toISOString()};if(a){const{error:d}=await c.from("installation_monitorings").update(i).eq("id",a);if(d){m(`Error: ${d.message}`,"error");return}}else{const{error:d}=await c.from("installation_monitorings").insert(i);if(d){m(`Error: ${d.message}`,"error");return}}m("Data monitoring berhasil disimpan","success"),o.hide(),t&&t()}function N(e){return new Promise((a,n)=>{const t=new FileReader;t.onload=()=>a(t.result),t.onerror=n,t.readAsDataURL(e)})}function O(e,a){new bootstrap.Modal(document.getElementById("mapModal")).show(),setTimeout(()=>{window.adminModalMap&&(window.adminModalMap.remove(),window.adminModalMap=null);const t=e.filter(l=>l.customers?.lat&&l.customers?.lng);if(t.length===0){alert("Tidak ada data PSB dengan koordinat yang valid untuk filter ini.");return}const o=L.map("admin-map").setView([t[0].customers.lat,t[0].customers.lng],12);window.adminModalMap=o,L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png").addTo(o),F(o);const s=document.querySelector("#mapModal .modal-title");if(s){const l=a==="All"?"Semua Status":a;s.innerHTML=`<i class="bi bi-map me-2"></i>Peta PSB — <span class="badge" style="background:${b(a)}">${l}</span> (${t.length} titik)`}const r=[];if(t.forEach(l=>{const i=b(l.status),d=L.marker([l.customers.lat,l.customers.lng],{icon:L.divIcon({className:"",html:R(i),iconSize:[28,36],iconAnchor:[14,36],popupAnchor:[0,-36]})}).addTo(o).bindPopup(z(l));r.push(d)}),r.length>1){const l=new L.featureGroup(r);o.fitBounds(l.getBounds().pad(.15))}},300)}function M(e,a){window.showAllPSBMap=()=>O(e,a)}function R(e){return`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 32" width="28" height="36">
    ...
    `;let t=document.getElementById("wo-status-summary");t||(t=document.createElement("div"),t.id="wo-status-summary",t.className="mb-3",e.parentNode.insertBefore(t,e)),await x(a,s=>{f=s,y()}),document.getElementById("add-wo-btn").onclick=()=>B(null),document.getElementById("refresh-wo-btn").onclick=()=>{x(a,s=>{f=s,y()})};const o=document.getElementById("map-button-wo");o&&(o.onclick=()=>{const s=_(f,g,$);M(s,g),window.showAllPSBMap()}),T(s=>{$=s,y()}),document.addEventListener("request-wo-confirmation",async s=>{const{woId:r}=s.detail,l=f.find(i=>i.id===r);if(l)E(l);else{const{data:i,error:d}=await c.from("work_orders").select("*").eq("id",r).single();i?E(i):m(`Work order with ID ${r} not found.`,"error")}})}function y(){I(f,g,a=>{g=a,y()});const e=_(f,g,$);M(e,g),S(e,a=>K(a),a=>E(a))}async function E(e){const a=document.getElementById("assignConfirmModal"),n=new bootstrap.Modal(a),t=document.getElementById("assign-wo-code"),o=document.getElementById("save-assign-confirm-btn");t.textContent=e.title||e.id;const s=document.getElementById("assign-employee-select");if(s){s.innerHTML='<option value="">-- Pilih Teknisi (opsional) --</option>';const{data:l}=await c.from("employees").select("id, name").eq("status","Aktif").order("name");l&&l.forEach(i=>{const d=document.createElement("option");d.value=i.id,d.textContent=i.name,s.appendChild(d)}),e.employee_id&&(s.value=e.employee_id)}const r=async()=>{o.disabled=!0,o.innerHTML='<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Menyimpan...';const l=s&&s.value||null,i={status:"confirmed"};l&&(i.employee_id=l);const{error:d}=await c.from("work_orders").update(i).eq("id",e.id);if(d){m(`Error: ${d.message}`,"error"),o.disabled=!1,o.innerHTML="Simpan";return}const{error:p}=await c.from("installation_monitorings").insert({work_order_id:e.id,employee_id:l,notes:"Work order dikonfirmasi oleh admin."});p?m(`Work order dikonfirmasi, tapi gagal membuat log monitoring: ${p.message}`,"warning"):m("Work order berhasil dikonfirmasi.","success"),n.hide();const u=document.getElementById("work-orders-list");x(u,w=>{f=w,y()})};o._clickHandler&&o.removeEventListener("click",o._clickHandler),o._clickHandler=r,o.addEventListener("click",o._clickHandler),a.addEventListener("hidden.bs.modal",()=>{o.disabled=!1,o.innerHTML="Simpan",o._clickHandler&&o.removeEventListener("click",o._clickHandler)},{once:!0}),n.show()}async function K(e){const a=new bootstrap.Modal(document.getElementById("crudModal")),n=document.getElementById("crudModalBody");n.innerHTML=`
